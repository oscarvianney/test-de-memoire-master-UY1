# USAGE
# python race_classification_dslbp.py
...
for imagePath in imagePaths:
	...
	while i <= image_gray.shape[0]-size_block :
		...
		while j <= image_gray.shape[1]-size_block:
			part_image_dslbp = operateur.diagonal_symmetric_lbp(image_gray[i:i+size_block-1,j:j+size_block-1])
			features = np.concatenate((features,np.histogram(part_image_dslbp.ravel(),bins=8,range=(0, 8))[0]))
